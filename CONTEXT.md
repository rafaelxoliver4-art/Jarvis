...
- **A registered `delegate_task` is not an *invoked* `delegate_task`.** The ElevenLabs brain LLM decides which tool to call; if a request can be satisfied by the primitive tools inline (e.g. "research X and save it" → `search_web` + `save_file`), it will often do that instead of delegating — even when the word "delegate" is spoken (voice test #2, 2026-06-01). Routing to `delegate_task` is a **prompt/description-design** problem: the dashboard tool description must position it for genuinely multi-step/autonomous work and de-overlap it from single-primitive tasks, and the agent's system prompt may need an explicit steering line. **Always verify which tool actually fired via `logs/usage_log.jsonl` / `logs/delegate_log.jsonl` — never trust Jarvis's spoken "I delegated that" (it narrates intent, not the executed mechanism).**
