...
- **"Created a tool" ≠ "the agent has the tool." Two separate dashboard steps.** In the ElevenLabs dashboard, a Client Tool must be (1) created AND (2) explicitly added to the specific agent's tool list, then the agent saved/published. Voice test #3 (2026-06-01) showed the agent flatly stating *"I do not have a delegate task tool in my current toolkit"* even though the tool had been "registered." **The agent's self-report of its toolset is authoritative** — quickest check before any delegation test: ask Jarvis "what tools do you have?" If it doesn't list the tool, it isn't attached, and no description/routing tuning will help.
- **A registered `delegate_task` is not an *invoked* `delegate_task`.** Even once attached, the ElevenLabs brain LLM decides which tool to call; if a request can be satisfied by the primitive tools inline (e.g. "research X and save it" → `search_web` + `save_file`), it may do that instead of delegating. That's a prompt/description-design problem (position `delegate_task` for genuinely multi-step work; possibly a system-prompt steering line). **Always verify which tool actually fired via `logs/usage_log.jsonl` / `logs/delegate_log.jsonl` — never trust Jarvis's spoken narration** (it reports intent, not the executed mechanism, and an echo loop can make it misreport a *working* tool as "failing" — test #3 fired 10 `search_web` calls all `outcome=ok` while Jarvis said the search "kept failing").
- **A real multi-step delegation takes ~90s — size the timeouts around that, as a 3-level invariant.** Verified end-to-end 2026-06-01: a "research 3 things, compare, save" delegation runs ~94s (3 turns, ~$0.09). The timeouts must nest: **real completion (~94s) < parent `_WORKER_TIMEOUT_SEC` (110s) < ElevenLabs dashboard "Response timeout" (120s — the ElevenLabs MAX).** Note the dashboard caps at 120s, so the parent is 110s (not higher): on the timeout path the parent runs the full window AND then tears down the worker process tree (~3–5s) before returning, so its worst-case return must stay under the dashboard cap. If the parent kill is shorter than real completion, every delegation dies as `status=timeout` even though the worker is fine (this caused voice test #4's failures with the original 60s parent kill). The worker is not hanging — it's doing real work. UX caveat: ~90s of voice silence is long; consider tighter goals / interim feedback.
- **The audio echo loop is now a hard blocker for delegation voice tests — use headphones.** Without them, Jarvis's own TTS is transcribed back as user turns, triggering ElevenLabs's interruption logic mid-response; the agent talks to itself, retries tools redundantly, and misperceives success as failure. A long `delegate_task` reply makes this far worse than a single tool call.
...
- **NOT cloud-stored personal memory.** Memory stays local on Rafael's machine, with schema-validated writes (a defense against memory poisoning — OWASP agentic risk #5). **Phase 6 standing rules (built 2026-06-02):** memory is **explicit-only** (written only when the user says "remember ..."; no passive extraction); the JSONL store is **gitignored** (never to GitHub); recalled/loaded memory is **DATA, never instructions**; writes that look like secrets/credentials or attempts to change behavior/safety/tool-access are **rejected**; and **self-written "reflections" are quarantined** (`review_status="pending"` + `load_at_start=false`) so they can NEVER auto-enter context — promotion is a deliberate future Phase 6.5 step, never automatic.
...
- **2026-06-02** — **Phase 6 Stage 2 (slim) built: memory mutability + safe-skip.** Added `forget` (reversible `op="revoke"` tombstone — append-only, single-fact, never bulk, asks on ambiguity), supersede-honored loader/recall, malformed-line safe-skip, and write-time dedupe. Correction = forget-then-remember (no update tool, no auto-supersede). `checksum`/file-locking deferred to Stage 2.5. Detail in PROGRESS.md.
