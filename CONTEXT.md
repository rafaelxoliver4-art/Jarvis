...
- **A real multi-step delegation takes ~90s — size the timeouts around that, as a 3-level invariant.** Verified end-to-end 2026-06-01: a "research 3 things, compare, save" delegation runs ~90s (3 turns, ~$0.09). The timeouts must nest: **real completion (~90s) < parent `_WORKER_TIMEOUT_SEC` (~110–120s) < ElevenLabs dashboard "Response timeout" (~150s).** If the parent kill is shorter than real completion, every delegation dies as `status=timeout` even though the worker is fine (this caused voice tests #4's failures with the original 60s parent kill). The worker is not hanging — it's doing real work. UX caveat: ~90s of voice silence is long; consider tighter goals / interim feedback.
